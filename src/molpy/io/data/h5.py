...
def frame_to_h5_group(
    ...
    if frame.box is not None:
        ...
        box_group.create_dataset("matrix", data=np.asarray(frame.box.matrix, dtype=np.float64))
        box_group.create_dataset("origin", data=np.asarray(frame.box.origin, dtype=np.float64))
