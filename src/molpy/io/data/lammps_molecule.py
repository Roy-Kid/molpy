...
class LammpsMoleculeReader(DataReader):
    ...
    def _read_json_format(self, frame: mp.Frame) -> mp.Frame:
        ...
        if 'coords' in data:
            ...
            for coord_entry in coords_data:
                ...
                positions[idx] = [float(coord_entry[1]), float(coord_entry[2]), float(coord_entry[3])]
    ...
    def _parse_json_connectivity(self, data: Dict, frame: mp.Frame, section: str) -> None:
    ...
    def _parse_native_sections(self, lines: List[str]) -> Tuple[Dict[str, Any], Dict[str, List[str]]]:
    ...
    def _parse_header_line(self, line: str, header_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'atoms' in line and not line.startswith('atoms'):
                ...
                header_info['center_of_mass'] = [float(parts[0]), float(parts[1]), float(parts[2])]
                return True
    ...
    def _parse_native_atoms(self, sections: Dict[str, List[str]], header_info: Dict[str, Any]) -> Dict[str, np.ndarray]:
    ...
    def _parse_native_connectivity(self, sections: Dict[str, List[str]], section_type: str) -> Optional[Dict[str, np.ndarray]]:
        ...
        for line in lines:
            ...
            if section_type == 'bonds' and len(parts) >= 4:
                ...
                ids.append(int(parts[0]))
                types.append(parts[1])
                atoms.append([int(parts[2]), int(parts[3]), int(parts[4]), int(parts[5])])
...
class LammpsMoleculeWriter(DataWriter):
    ...
    def _write_json_format(self, frame: mp.Frame) -> None:
        ...
        data = {
            ...
            "format": "molecule",
            "revision": 1,
            "title": frame.metadata.get('title', f"Molecule template written by molpy on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"),
    ...
    def _write_native_connectivity_section(self, lines: List[str], block: mp.Block, section_type: str) -> None:
