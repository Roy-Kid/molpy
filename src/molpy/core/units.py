...
from pint import UnitRegistry, Quantity, Unit as PintUnit, DimensionalityError
...
class Unit:
    ...
    _base_keys = ['mass', 'length', 'time', 'energy', 'temperature',
                  'charge', 'pressure', 'viscosity', 'electric_field',
                  'dipole', 'density']
    ...
    _lammps_presets: Dict[UnitSystemName, Dict[str, Quantity]] = {
        ...
        'micro': {
            ...
            'energy':      1.0 * _registry.picogram * _registry.micrometer ** 2 / _registry.microsecond ** 2,
            'temperature': 1.0 * _registry.kelvin,
            'charge':      1.6021765e-7 * _registry.coulomb,
            'pressure':    1.0 * _registry.picogram / (_registry.micrometer * _registry.microsecond ** 2),
            'viscosity':   1.0 * _registry.picogram / (_registry.micrometer * _registry.microsecond),
        ...
        'nano': {
            ...
            'energy':      1.0 * _registry.attogram * _registry.nanometer ** 2 / _registry.nanosecond ** 2,
            'temperature': 1.0 * _registry.kelvin,
            'charge':      1.0 * _registry.elementary_charge,
            'pressure':    1.0 * _registry.attogram / (_registry.nanometer * _registry.nanosecond ** 2),
            'viscosity':   1.0 * _registry.attogram / (_registry.nanometer * _registry.nanosecond),
    ...
    def _to_reduced_complex(self, q_base: Quantity) -> float:
        ...
        fundamental_dims = ['[mass]', '[length]', '[time]', '[temperature]', 
                           '[substance]', '[current]', '[luminosity]']
    ...
    def _matches_preset(self, preset: Dict[str, Quantity]) -> bool:
        ...
        for key, expected_qty in preset.items():
            ...
            if abs(expected_base.magnitude - actual_base.magnitude) > 1e-10 * abs(expected_base.magnitude):
                return False
        return True
        
    def _compare_unit_dicts(self, dict1: Dict[str, Quantity], dict2: Dict[str, Quantity]) -> bool:
