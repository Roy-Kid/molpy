...
class Systemic(Wrapper):
    ...
    def __init__(
        self, wrapped, box: Box | None = None, forcefield: ForceField | None = None
    ):
...
class StructSystem(Systemic):
    ...
    def __init__(
        self,
        struct: Struct | Wrapper[Struct] | None = None,
        box: Box | None = None,
        forcefield: ForceField | None = None,
    ):
        if struct is None:
...
class PeriodicSystem(Wrapper):
    ...
    def __init__(self, system: FrameSystem | StructSystem):
        ...
        if not system.box.is_periodic:
            raise ValueError(
                "System must have periodic boundary conditions for supercell operations"
            )
    ...
    def make_supercell(
        self, transformation_matrix: list[list[int]]
    ) -> "PeriodicSystem":
        ...
        if len(transformation_matrix) != 3 or any(
            len(row) != 3 for row in transformation_matrix
        ):
    ...
    def _create_frame_supercell(self, matrix: np.ndarray, new_box: Box) -> FrameSystem:
        ...
        return FrameSystem(
            frame=new_frame, box=new_box, forcefield=self._wrapped.forcefield
        )

    def _create_struct_supercell(
        self, matrix: np.ndarray, new_box: Box
    ) -> StructSystem:
        ...
        return StructSystem(
            struct=new_struct, box=new_box, forcefield=self._wrapped.forcefield
        )
            ...
            if pos_var is not None:
                ...
                for translation in translations:
                    ...
                    for i in range(n_atoms):
                        replicated_positions[atom_idx] = (
                            original_positions[i] + real_translation
                        )
                        atom_idx += 1
                ...
                for var_name in block:
                    if var_name != pos_var and len(block[var_name]) == n_atoms:
                        ...
                        if value.ndim > 1:
                            new_block_data[var_name] = np.tile(
                                value, (supercell_size, 1)
                            )
                        else:
    ...
    def _replicate_struct(
        self, struct: Struct | Wrapper, matrix: np.ndarray
    ) -> Struct | Wrapper:
    ...
    def add_vacuum(
        self, vacuum_thickness: float, direction: str = "z"
    ) -> "PeriodicSystem":
    ...
    def _create_vacuum_system(
        self, vacuum_thickness: float, direction: str
    ) -> FrameSystem | StructSystem:
