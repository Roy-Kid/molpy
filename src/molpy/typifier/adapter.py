...
def build_mol_graph(structure: "Atomistic") -> tuple[Graph, dict[int, int], dict[int, int]]:
    ...
    if not isinstance(structure, Atomistic):
        raise TypeError(f"Expected Atomistic, got {type(structure)}")
...
def _extract_atom_attributes(atom: "Atom", structure: "Atomistic") -> dict[str, Any]:
    ...
    # Aromaticity
    attrs["is_aromatic"] = bool(atom.get("is_aromatic", 
...
def _extract_bond_attributes(bond: "Bond", structure: "Atomistic") -> dict[str, Any]:
    ...
    if isinstance(order, str):
        ...
        attrs["order"] = int(order)
        attrs["is_aromatic"] = bool(bond.get("is_aromatic", 
                                             bond.get("aromatic", False)))
...
def _compute_derived_properties(g: Graph, structure: "Atomistic", 
                                 atom_to_vs: dict) -> None:
...
def _detect_rings(g: Graph, max_ring_size: int = 8) -> None:
    ...
    # Mark edges in rings
    for e in g.es:
        source_cycles = g.vs[e.source]["cycles"] if "cycles" in g.vs[e.source].attributes() else set()
        target_cycles = g.vs[e.target]["cycles"] if "cycles" in g.vs[e.target].attributes() else set()
