...
class LayeredTypingEngine:
    ...
    def __init__(self, patterns: Dict[str, SMARTSGraph]):
        ...
        for level in range(max_level + 1):
            ...
            if is_circular:
                ...
                type_assignments = self._resolve_circular(
                    level_patterns, mol_graph, vs_to_atomid, 
                    type_assignments, max_iterations
                )
            else:
    ...
    def _resolve_level(self,
                       ...
                       mol_graph: Graph,
                       vs_to_atomid: dict[int, int],
                       current_assignments: dict[int, str]) -> dict[int, str]:
    ...
    def _resolve_circular(self,
                          level_patterns: list[SMARTSGraph],
                          mol_graph: Graph,
                          vs_to_atomid: dict[int, int],
                          current_assignments: dict[int, str],
                          max_iterations: int) -> dict[int, str]:
    ...
    def get_explain_data(self, 
                        mol_graph: Graph,
                        vs_to_atomid: dict[int, int]) -> dict:
