...
class LJ126(PairPotential):
    ...
    def __init__(self, epsilon: Union[float, NDArray[np.float64]], sigma: Union[float, NDArray[np.float64]]):
        """
    ...
    def _extract_data_from_frame(self, frame):
        ...
        atoms = frame["atoms"]
        coords = atoms["xyz"] if "xyz" in atoms else np.column_stack([atoms["x"], atoms["y"], atoms["z"]])
        atom_types = atoms["type"] if "type" in atoms else np.zeros(len(coords), dtype=np.int32)
    ...
    def calc_energy(self, *args, **kwargs):
        ...
        if len(args) == 1 and hasattr(args[0], '__getitem__') and 'atoms' in args[0]:
            ...
            raise ValueError("calc_energy expects either a Frame or (dr_norm, pair_types)")
        
        if len(pair_types) == 0:
            return np.array([]).reshape(0, 1)
            
        if np.any(pair_types >= len(self.epsilon)) or np.any(pair_types < 0):
            raise ValueError(f"pair_types contains invalid indices. Must be in range [0, {len(self.epsilon)})")
    ...
    def calc_force(self, *args, **kwargs):
        ...
        if len(args) == 1 and hasattr(args[0], '__getitem__') and 'atoms' in args[0]:
            ...
            raise ValueError("calc_force expects either a Frame or (dr, dr_norm, pair_types)")
        
        if len(pair_types) == 0:
            return np.array([]).reshape(0, 3)
            
        if dr.shape[0] != dr_norm.shape[0] or dr.shape[0] != pair_types.shape[0]:
            raise ValueError("dr, dr_norm, and pair_types must have the same number of pairs")
