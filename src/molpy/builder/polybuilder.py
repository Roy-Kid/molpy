...
class PolymerBuilder:
    ...
    def build(
        ...
        for i, monomer_name in enumerate(seq):
            monomer_template = self.monomers[
                monomer_name
            ]()  # Get the original monomer template
    ...
    def _apply_proportional_transformation(
        ...
        rotation_matrix = rotation_matrix_from_vectors(
            native_direction, target_direction
        )
    ...
    def _get_native_direction(
        self, positions: np.ndarray, anchor_rule: AnchorRule
    ) -> np.ndarray:
