...
class CrystalSite(BaseSiteProvider):
    ...
    def __init__(
        self, cell: Sequence[Sequence[float]], basis: Iterable[Sequence[float]]
    ):
...
class CrystalBuilder(StructBuilder):
    ...
    def build(
        self, region: Region, template: Atomistic, name: str = "crystal"
    ) -> Atomistic:
        """
    ...
    def fill_lattice(
        self, region: Region, template: Atomistic, name: str = "crystal_cell"
    ) -> Atomistic:
        """
...
class RandomWalkSite:
    ...
    def gen_site(
        ...
        for step in range(n_steps):
            ...
            if not valid_step_found:
                print(
                    f"Warning: Could not find valid step at position {step+1}, "
    ...
    def _is_valid_position(
        self, new_pos: np.ndarray, existing_positions: list, exclusion_radius: float
    ) -> bool:
...
class RandomWalkBuilder(StructBuilder):
    ...
    def build(
        ...
        positions = self.site_provider.gen_site(
            n_steps=n_monomers - 1, start_pos=start_position
        )
