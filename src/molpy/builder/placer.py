...
class LinearOrienter:
    ...
    def get_orientation(
        ...
        # Get positions
        left_anchor_pos = np.array(left_anchor.get('pos', left_anchor.get('xyz', [0., 0., 0.])))
        right_anchor_pos = np.array(right_anchor.get('pos', right_anchor.get('xyz', [0., 0., 0.])))
    ...
    def _get_port_direction(self, monomer: Monomer, port: Port) -> np.ndarray:
        ...
        """
        anchor = port.target
        anchor_pos = np.array(anchor.get('pos', anchor.get('xyz', [0., 0., 0.])))
        ...
        for bond in bonds:
            if bond.itom == anchor:
                neighbor = bond.jtom
                neighbor_pos = np.array(neighbor.get('pos', neighbor.get('xyz', [0., 0., 0.])))
                break
        ...
        if neighbor_pos is not None:
            ...
            positions = [np.array(a.get('pos', a.get('xyz', [0., 0., 0.]))) for a in atoms]
    ...
    def _rotation_matrix_from_vectors(self, vec_from: np.ndarray, vec_to: np.ndarray) -> np.ndarray:
        """
...
class Placer:
    ...
    def place_monomer(
        ...
        self._apply_transform(right_monomer, translation, rotation, pivot=right_anchor_pos)
    ...
    def _apply_transform(
        ...
        if pivot is None:
            positions = []
            for atom in atoms:
                pos_key = 'pos' if 'pos' in atom.data else 'xyz'
                pos = np.array(atom.get(pos_key, [0., 0., 0.]))
