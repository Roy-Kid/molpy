...
class AtomPrimitiveIR:
    ...
    """
    type: Literal["symbol", "atomic_num", "neighbor_count", "ring_size", 
                  "ring_count", "has_label", "matches_smarts", "wildcard"]
    value: str | int | SmartsIR | None = None
    id: int = field(default_factory=lambda: id(AtomPrimitiveIR), compare=False, repr=False)
...
class AtomExpressionIR:
    ...
    id: int = field(default_factory=lambda: id(AtomExpressionIR), compare=False, repr=False)
...
class SmartsTransformer(Transformer):
    ...
    def atom_id(self, children: list) -> AtomPrimitiveIR:
        ...
        Can be:
            ...
            - R + N (ring count)
        """
        # Filter out operator tokens like #, X, r, R, $(, )
        filtered = [c for c in children if not (isinstance(c, Token) and c.value in {"#", "X", "r", "R", "$(", ")", "$", "("})]
    ...
    def atom(self, children: list) -> SmartsAtomIR:
        ...
        filtered = [c for c in children if not (isinstance(c, Token) and c.type in {"LSQB", "RSQB"})]
            ...
            if isinstance(item, SmartsAtomIR):
                ...
                if item.label is not None:
                    ...
                    if label_str in self.ring_openings:
                        ...
                        ir.bonds.append(SmartsBondIR(opening_atom, item, bond_type or "-"))
                ...
                if isinstance(branch_ir, SmartsIR) and branch_ir.atoms:
                    if active_atom is not None:
                        ...
                        ir.bonds.append(SmartsBondIR(active_atom, head_atom, branch_bond_type or "-"))
...
def _ir_to_smarts_string(ir: SmartsIR) -> str:
    ...
    def atom_to_str(atom: SmartsAtomIR) -> str:
        ...
        # Simple primitive
        if isinstance(expr, AtomExpressionIR) and expr.op == "primitive" and len(expr.children) == 1:
    ...
    def dfs(atom: SmartsAtomIR, parent_id: int | None = None) -> str:
        ...
        for neighbor_id, bond_type in neighbors:
