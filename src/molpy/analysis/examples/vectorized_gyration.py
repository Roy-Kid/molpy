...
from molpy.analysis.base import (
    Compute,
    ComputeContext,
    get_entities_from_frame,
    get_selected_entities,
)
from molpy.analysis.selection.expression import ExpressionSelection, SelectResult
...
class MockMolecularFrame(Frame):
    ...
    def __init__(self, n_molecules: int = 3, atoms_per_mol: int = 10):
        ...
        self["atoms"] = {
            "xyz": np.random.random((total_atoms, 3))
            * 10,  # Random positions in 10x10x10 box
            "id": np.arange(total_atoms),
            "mol_id": np.repeat(
                np.arange(n_molecules), atoms_per_mol
...
def demonstrate_mapped_gyration():
    ...
    selection = ExpressionSelection(
        "first_two_molecules", select_first_two, select_field="molecules"
    )
    ...
    if selected_ids is not None:
        mapped_result = gyration_calc.map(
            context, entity_ids=selected_ids, selection_field="molecules"
        )
    ...
    specific_mapped = gyration_calc.map(
        context, entity_ids=specific_ids, selection_field="molecules"
    )
    ...
    if all_mol_ids is not None:
        ...
        subset_mapped = gyration_calc.map(
            context, entity_ids=subset_ids, selection_field="molecules"
        )
