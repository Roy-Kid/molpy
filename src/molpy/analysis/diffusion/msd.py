...
class DirectMSD(Compute):
    ...
    def compute(self, xyz: np.ndarray) -> np.ndarray:
        ...
        _msd_result.append(np.linalg.norm(xyz - xyz[[0], :, :], axis=-1) ** 2)

        return np.array(_msd_result)
...
class WindowedMSD(Compute):
    ...
    def compute(self, xyz: np.ndarray) -> np.ndarray:
        ...
        N = xyz.shape[0]
        D = np.square(xyz).sum(axis=2)
        D = np.append(D, np.zeros(xyz.shape[:2]), axis=0)
        Q = 2 * D.sum(axis=0)
        S1 = np.zeros(xyz.shape[:2])
        for m in range(N):
            Q -= D[m - 1, :] + D[N - m, :]
            S1[m, :] = Q / (N - m)
        ...
        for i in range(xyz.shape[2]):
            corrs.append(_autocorrelation(xyz[:, :, i]))
