...
LAMMPS log files are different from data and trajectory files: they describe what LAMMPS did during each `run`, not where atoms were. The molrs parser keeps that structure: a `LammpsLog` holds the header, one `LammpsRun` per `run`, and the raw text of anything not represented by a dedicated field.
...
| Format | Read | Write | Notes |
|--------|------|-------|-------|
| LAMMPS log | `read_lammps_log` | — | `LammpsLog` → `LammpsRun` → `LammpsThermo`, parsed in molrs |
