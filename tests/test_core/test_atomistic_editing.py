...
def _ethane() -> Atomistic:
    ...
    h = [mol.def_atom(symbol="H", type="HC", xyz=[0.0, float(i), 0.0]) for i in range(6)]
...
class TestRenameType:
    def test_rename_atom_type(self):
        ...
        assert n == 2
        assert all(
            a.get("type") != "CT" for a in mol.atoms if a.get("symbol") == "C"
        )
        assert (
            sum(1 for a in mol.atoms if a.get("type") == "opls_135") == 2
        )
    ...
    def test_rename_bond_type(self):
        ...
        assert n == 6
        assert all(
            b.get("type") != "CT-HC" for b in mol.bonds
        )
...
class TestSetProperty:
    def test_set_property_callable(self):
        mol = _ethane()
        n = mol.set_property(
            lambda a: a.get("symbol") == "H", "charge", 0.06
        )
        assert n == 6
        assert all(
            a.get("charge") == 0.06 for a in mol.atoms if a.get("symbol") == "H"
        )
        assert all(
            "charge" not in a.data
            for a in mol.atoms
            if a.get("symbol") == "C"
        )
