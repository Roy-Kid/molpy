...
from molpy.core.system import FrameSystem, PeriodicSystem, StructSystem
...
class TestPeriodicSystem:
    ...
    def setup_method(self):
        ...
        self.frame_system = FrameSystem(
            frame=self.frame, box=self.box, forcefield=self.ff
        )
    ...
    def test_add_vacuum_default_z(self):
        ...
        np.testing.assert_array_equal(
            original_frame["atoms"]["xyz"], new_frame["atoms"]["xyz"]
        )
    ...
    def test_add_vacuum_preserves_properties(self):
        ...
        for var_name in original_frame["atoms"]:
            np.testing.assert_array_equal(
                original_frame["atoms"][var_name], new_frame["atoms"][var_name]
            )
    ...
    def test_add_vacuum_maintains_periodicity(self):
        ...
        np.testing.assert_array_equal(periodic_system.box.pbc, vacuum_system.box.pbc)
...
class TestPeriodicSystemIntegration:
    ...
    def test_zno_surface_example(self):
        ...
        atom_block = Block(
            {"xyz": positions, "element": elements, "id": np.arange(1, 5)}
        )
    ...
    def test_chaining_supercells(self):
        ...
        frame = Frame()
        atom_block = Block(
            {"xyz": np.array([[0.0, 0.0, 0.0]]), "type": np.array(["C"])}
        )
        frame["atoms"] = atom_block
    ...
    def test_zno_surface_with_vacuum(self):
        ...
        np.testing.assert_array_equal(
            supercell_frame["atoms"]["xyz"], new_frame["atoms"]["xyz"]
        )
