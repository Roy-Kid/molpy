...
class TestSpatialMixin:
    ...
    def test_scale(self) -> None:
        asm = Atomistic()
        a = Atom({"pos": [1.0, 1.0, 1.0]})
        asm.add_entity(a)
        asm.scale(2.0, about=[0.0, 0.0, 0.0])
        assert a["pos"] == [2.0, 2.0, 2.0]

    def test_align_origin_only(self) -> None:
        asm = Atomistic()
        a = Atom({"pos": [1.0, 0.0, 0.0]})
        b = Atom({"pos": [3.0, 4.0, 0.0]})
        asm.add_entity(a)
        asm.add_entity(b)
        asm.align(a, b)
        assert a["pos"] == [3.0, 4.0, 0.0]

    def test_align_with_dirs_and_flip(self) -> None:
        asm = Atomistic()
        a = Atom({"pos": [0.0, 0.0, 0.0]})
        b = Atom({"pos": [0.0, 0.0, 0.0]})
