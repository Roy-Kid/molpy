...
class TestLammpsMoleculeReader:
    ...
    def test_read_ethane_native(self, test_files):
        ...
        # Check masses
        expected_masses = np.array([12.011, 12.011, 1.008, 1.008, 1.008, 1.008, 1.008, 1.008])
    ...
    def test_empty_file_error(self):
        ...
        try:
            reader = LammpsMoleculeReader(temp_path)
    ...
    def test_missing_types_section_json(self):
        ...
        try:
            reader = LammpsMoleculeReader(temp_path)
            with pytest.raises(ValueError, match="JSON molecule file must contain 'types' section"):
                reader.read()
        finally:
            temp_path.unlink()
    
    def test_missing_types_section_native(self):
        ...
        try:
            reader = LammpsMoleculeReader(temp_path)
            with pytest.raises(ValueError, match="Native molecule file must contain Types section"):
                reader.read()
        finally:
            temp_path.unlink()
...
class TestLammpsMoleculeWriter:
    ...
    def test_write_native_format(self, test_files):
        ...
        try:
            writer = LammpsMoleculeWriter(temp_path, format_type="native")
            writer.write(frame)
            
            # Read back and compare
            reader2 = LammpsMoleculeReader(temp_path)
            frame2 = reader2.read()
    ...
    def test_write_json_format(self, test_files):
        ...
        try:
            writer = LammpsMoleculeWriter(temp_path, format_type="json")
            writer.write(frame)
    ...
    def test_roundtrip_native_to_json(self, test_files):
        ...
        try:
            ...
            temp_native_path.unlink()
    
    def test_write_without_atoms_error(self):
    ...
    def test_metadata_preservation(self, test_files):
...
class TestIntegrationWithMolpyIO:
    ...
    def test_write_lammps_molecule_function(self, test_files):
        ...
        try:
            mp.io.write_lammps_molecule(temp_path, frame, format_type="native")
            
            # Read back and verify
            frame2 = mp.io.read_lammps_molecule(temp_path)
            assert frame2['atoms'].nrows == frame['atoms'].nrows
            
        finally:
            temp_path.unlink()
    
    def test_write_json_format_function(self, test_files):
        ...
        try:
            mp.io.write_lammps_molecule(temp_path, frame, format_type="json")
