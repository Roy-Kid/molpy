...
def make_atom(
    expr_type: Literal["symbol", "atomic_num", "neighbor_count", "ring_size", 
                       "ring_count", "has_label", "matches_smarts", "wildcard"] = "symbol",
    value: str | int = "C",
    label: int | None = None
):
    """Helper to create a SmartsAtomIR with a simple primitive."""
    prim = AtomPrimitiveIR(type=expr_type, value=value)
    expr = AtomExpressionIR(op="primitive", children=[prim])
    return SmartsAtomIR(expression=expr, label=label)


def make_smarts_ir(atom_specs, bond_tuples):
    """
    Helper to create SmartsIR from specifications.
    
    Args:
        atom_specs: list of dict or tuple describing atoms
            dict: {type: str, value: any, label: int}
            tuple: (type, value) or (type, value, label)
        bond_tuples: list of (start_idx, end_idx, bond_type)
    """
    atoms = []
    for spec in atom_specs:
        if isinstance(spec, dict):
            atoms.append(make_atom(**spec))
        elif isinstance(spec, tuple):
            if len(spec) == 2:
                atoms.append(make_atom(spec[0], spec[1]))
            else:
                atoms.append(make_atom(spec[0], spec[1], spec[2]))
        else:
            atoms.append(make_atom(value=spec))
    
    bonds = [SmartsBondIR(atoms[i], atoms[j], b) for (i, j, b) in bond_tuples]
    return SmartsIR(atoms=atoms, bonds=bonds)
...
class TestBranches:
    ...
    def test_simple_branch(self, parser):
        ...
        # Check connectivity
        assert (atom_ids[0], atom_ids[1]) in bond_pairs or (atom_ids[1], atom_ids[0]) in bond_pairs
        assert (atom_ids[0], atom_ids[2]) in bond_pairs or (atom_ids[2], atom_ids[0]) in bond_pairs
...
# ===================================================================
#   Integration Tests with RDKit (if available)
# ===================================================================

class TestRDKitIntegration:
    """Test conversion to RDKit Mol objects."""
    ...
    def test_simple_conversion(self, parser):
        """Test converting simple SMARTS to RDKit Mol."""
        try:
            from molpy.parser.smarts import smartsir_to_mol
        except ImportError:
            pytest.skip("RDKit not available")
    ...
    def test_atomic_number_conversion(self, parser):
        """Test converting atomic number pattern to RDKit."""
        try:
            from molpy.parser.smarts import smartsir_to_mol
        except ImportError:
            pytest.skip("RDKit not available")
    ...
    def test_or_pattern_conversion(self, parser):
        """Test converting OR pattern to RDKit."""
        try:
            from molpy.parser.smarts import smartsir_to_mol
        except ImportError:
            pytest.skip("RDKit not available")
...
try:
    ...
    class TestPropertyBased:
        ...
        @given(st.sampled_from(["C", "N", "O", "S", "P", "F", "Cl", "Br"]))
        def test_single_atom_roundtrip(self, symbol, parser):
            """Test that any single atom can be parsed."""
            ir = parser.parse_smarts(symbol)
            assert len(ir.atoms) == 1
            prim = ir.atoms[0].expression.children[0]
            assert prim.value == symbol
        
        @given(st.integers(min_value=1, max_value=118))
        def test_atomic_number_roundtrip(self, atomic_num, parser):
            """Test that any atomic number can be parsed."""
            smarts = f"[#{atomic_num}]"
            ir = parser.parse_smarts(smarts)
            assert len(ir.atoms) == 1
            prim = ir.atoms[0].expression.children[0]
            assert prim.type == "atomic_num"
            assert prim.value == atomic_num
