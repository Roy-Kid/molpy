...
class TestNew:
    ...
    def test_parse_new_with_chain(self):
        doc = parse_string(
            "w3 = new TIP3P.move(1,2,3).rot(45, 0, 0, 1).scale(1.5)\n"
        )
        new = doc.statements[0]
        assert isinstance(new, NewStmt)
...
class TestTip3pFixture:
    def test_parse_tip3p(self):
        ...
        sections = {
            s.section for s in cls.statements
            if isinstance(s, (WriteBlock, WriteOnceBlock))
        }
        assert {"Data Masses", "In Charges", "In Settings",
                "Data Atoms", "Data Bonds", "Data Angles"} <= sections
