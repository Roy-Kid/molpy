...
class TestBigSmilesToMonomer:
    ...
    def test_simple_monomer_topology(self):
        ...
        for atom in atomistic.atoms:
            assert 'pos' not in atom or atom['pos'] is None
...
class TestDescriptorToPortName:
    ...
    def test_indexed_descriptors(self):
        ...
        assert descriptor_to_port_name(BondDescriptorIR(symbol="$", index=3)) == "branch_3"
...
class TestCoordinateBinding:
    ...
    def test_coordinate_binding(self):
        ...
        # Verify
        assert monomer.unwrap().atoms[0]['pos'] == (0.0, 0.0, 0.0)
        assert monomer.unwrap().atoms[1]['pos'] == (1.5, 0.0, 0.0)
