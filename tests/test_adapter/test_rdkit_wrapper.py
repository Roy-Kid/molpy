...
class TestRDKitWrapperSymbolHandling:
    ...
    def test_generate_3d_symbol_preservation(self):
        ...
        assert len(atoms) >= 5, f"Should have at least 5 heavy atoms"
...
class TestRDKitWrapperAtomMapping:
    ...
    def test_atom_map_after_add_hydrogens(self):
        ...
        assert heavy_count >= 3, f"Expected at least 3 heavy atoms mapped, got {heavy_count}"
    ...
    def test_coordinate_sync_after_optimization(self):
        ...
        for i, atom in enumerate(atoms):
            ...
            pos_sum = abs(xyz[0]) + abs(xyz[1]) + abs(xyz[2])
        ...
        non_zero_count = sum(
            1 for a in atoms 
            if sum(abs(x) for x in a.get("xyz", [0, 0, 0])) > 0.1
        )
